...
class IoUPotoMatchloss(SimilarityMetric):
    def __init__(self, num_anchors: int, alpha: float=0.8) -> None:
        super().__init__()
    ...
    def get_matchloss(
        ...
        # Match quality: IoU^α * objectness^(1-α)
        match_loss_before_prior = iou.float_power(self.alpha) * post_activation_o_b_posi.float_power(1 - self.alpha)
